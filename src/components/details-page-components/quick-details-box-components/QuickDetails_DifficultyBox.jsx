                    ...
                    <div className="mt-2 w-32 h-7 flex justify-around items-center border-white border-2 border-solid rounded-full px-1">
                        {props.source === 1 ? (
                            femLevels.map((item, index) => (
                                <span key={index} className={`relative w-6 h-6 flex justify-center items-center font-extrabold text-lg leading-[18px] ${props.difficulty == index + 1 ? item.highlightColor +" " +item.textColor +" "+textShadow : 'text-white opacity-50'} rounded-full`}>
                                    {item.level}
                                </span>
                            ))
                        ) : props.source === 6 ? (
